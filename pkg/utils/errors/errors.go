...
// NotFound returns a "not found" error with additional context
func NotFound(message string) error {
	return fmt.Errorf("%w: %s", ErrNotFound, message)
}

// AlreadyExists returns an "already exists" error with additional context
func AlreadyExists(message string) error {
	return fmt.Errorf("%w: %s", ErrAlreadyExists, message)
}

// InvalidInput returns an "invalid input" error with additional context
func InvalidInput(message string) error {
	return fmt.Errorf("%w: %s", ErrInvalidInput, message)
}

// Internal returns an "internal error" with additional context
func Internal(message string) error {
	return fmt.Errorf("%w: %s", ErrInternal, message)
}

// Unavailable returns a "service unavailable" error with additional context
func Unavailable(message string) error {
	return fmt.Errorf("%w: %s", ErrUnavailable, message)
}

// Timeout returns a "timeout" error with additional context
func Timeout(message string) error {
	return fmt.Errorf("%w: %s", ErrTimeout, message)
}

// Permission returns a "permission denied" error with additional context
...
// As unwraps an error and checks if it matches a specific type
func As(err error, target interface{}) bool {
